//将多个文件的extent聚合在一起发送，而不是逐个inode发送
...
#define FIEMAPARRAYSIZE 88*2000 //发送的buffer设置为多大
char* fiemap_array = NULL;
unsigned long total_fiemaparraysize = 0; //表示现在fiemap_array中有多少字节的数据，作为统计和索引地址使用
...
void isFile(char* path)
{
    ...
    { 
        ...
        result->fm_start = total_file; //第一个extent里面的这个值是1不是0
        result->fm_length = total_fiemaparraysize+buf_size;

        //检查是否超过了大小
        if(buf_size + total_fiemaparraysize > FIEMAPARRAYSIZE){
            printf("size overflow!!!\n");
            exit(0);
        }

        //copy extents
        memcpy(fiemap_array+total_fiemaparraysize,(char*)result,buf_size);
        total_fiemaparraysize += buf_size;
...
int main(int argc, char *argv[]){
    ...
    struct timeval begin, end, tmp_begin, tmp_end, getdone;

    gettimeofday(&begin, 0);

    //初始化buffer
    fiemap_array = (char*)malloc(FIEMAPARRAYSIZE);
    memset(fiemap_array, 0, FIEMAPARRAYSIZE);

    //遍历整个文件夹,在这个过程中会拷贝所有fiemap到fiemap_array中
    isFile(argv[1]);
    //修改第一个fiemap，标识一共有多少个文件和总大小
    ((struct fiemap*)fiemap_array)->fm_start = total_file;
    ((struct fiemap*)fiemap_array)->fm_extent_count = total_fiemaparraysize; 

    gettimeofday(&getdone, 0);

    //发送所有fiemap
    //send to cosmos
    if(is_send){
        open_unvme();
        DMA_Send("/dev/nvme0n1",fiemap_array,total_fiemaparraysize);
        close_unvme();
    }
    ...
    printf("total DMA %luB(%.3f 4KB) data\n", total_fiemaparraysize,total_fiemaparraysize/4096.0);
    return 0;
}